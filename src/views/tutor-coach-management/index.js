...
function TutorCoachManagement() {
  ...
  const handleStatus = async () => {
    try {
      ...
      console.log(index)
      const response = await Get(COACH.toggleStatus  + selectedUser._id , token,{});
      ...
      render: (item) => (
        <FaEye
          style={{ fontSize: "16px", color: "#203453",  cursor: "pointer" }}
             onClick={() => navigate("/tutor-coach-management/" + item )}
        />
      ),
    },
  ];
  ...
  return (
    <Layout className="configuration">
      ...
      <Row style={{ padding: "10px 20px 0" }}>
          <h1 className="pageTitle">Tutor / Coach Management</h1>
        </Row>
...
export default TutorCoachManagement;
