...
import { NOTIFICATION, ADMIN } from "../../config/constants";
...
function Notifications() {
  ...
  const handleDelete = async () => {
    try {
      const response = await Get(ADMIN.deleteAllAdminNotifications , token,{});
      if(response.status){
  ...
  const getAllNotifications = (pageNumber, pageSize,status, reset = false) => {
    setLoading(true)
    try {
      Get(NOTIFICATION?.getAllAdminNotifications, token, {
        page: pageNumber
          ? pageNumber.toString()
...
<Row style={{ padding: "10px 20px" }}>
    ...
    >
      <h1 className="pageTitle">My Notifications</h1>
    </Col>
    
  </Row>
  <Row  gutter={20} style={{ padding: "10px 20px" }}>
                <Col
                  xs={24}
                  md={6}
                  style={{
