...
import { FaSearch,FaEdit, FaFilter, FaCaretDown, FaTrash,FaEye } from "react-icons/fa";
...
function ArticleManagement() {
  ...
  const resetFilter = () => {
    ...
    getCategories(paginationConfig.pageNumber, paginationConfig.limit, "", true);
  };
  ...
  const handleStatus = async () => {
    try {
      const index = categories.findIndex((cat) => cat._id == selectedCategory._id);

      console.log(index)
      const response = await Post(ARTICLE.edit + selectedCategory._id ,{status:!selectedCategory.status}, token,{});
  ...
  const getCategories = async (pageNumber, pageSize, search, reset = false) => {
    setLoading(true);
    try {
      const response = await Get(ARTICLE.get, token, {
        page: pageNumber
          ? pageNumber.toString()
    ...
    <Layout className="configuration">
      <div className="boxDetails">
        <Row style={{ padding: "10px 20px" }}>
          ...
          >
           <h1 className="pageTitle">Article Management</h1>
          </Col>
          <Col
            xs={24}
            md={12}
